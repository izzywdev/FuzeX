...
// Postgres DDL (CREATE DATABASE / DROP DATABASE) cannot parameterize an
// IDENTIFIER the way `query(text, params)` parameterizes a VALUE — `$1`
// only binds values, never a database/table/column name. The database name
// here is generated by THIS file (never user/network input), but a
// defensible fix means never trusting that by convention alone: validate
// the generated name against a strict allowlist before it is ever
// interpolated, and double-quote it as a Postgres identifier. This is what
// clears the `node-postgres-sqli` finding for real, rather than suppressing
// it on trust.
const SAFE_DB_NAME = /^[a-z][a-z0-9_]{0,62}$/;

/**
 * Validates `name` against SAFE_DB_NAME and returns it double-quoted as a
 * Postgres identifier (embedded double-quotes doubled per the SQL standard,
 * belt-and-braces since the allowlist already forbids `"` outright).
 * Throws rather than interpolating anything that doesn't match.
 */
function quoteIdent(name) {
  if (!SAFE_DB_NAME.test(name)) {
    throw new Error(`refusing to use "${name}" as a Postgres identifier — fails SAFE_DB_NAME allowlist`);
  }
...
async function provisionEphemeralDatabase(baseDatabaseUrl) {
  ...
  const identifier = quoteIdent(dbName); // throws before any interpolation if dbName is ever not the shape we generated
  const admin = new Client({ connectionString: baseDatabaseUrl });
  await admin.connect();
  try {
    // nosemgrep: javascript.lang.security.audit.sqli.node-postgres-sqli -- DATABASE identifier can't be parameterized in Postgres DDL; name is a validated internal constant (SAFE_DB_NAME + quoteIdent above), never user input.
