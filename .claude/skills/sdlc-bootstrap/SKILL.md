...
6. **Automation + gate** — from `workflow-templates/` (incl. `governance-nightly.yml` for the nightly reconciliation sweep **and `nightly-integration.yml`** for nightly integration tests + draft-PR auto-fix — give each repo a **staggered cron minute** so the fleet doesn't thunder). Ensure the **`@claude` handler (`claude.yml`) + the agent set are installed** so the nightly's `@claude` build-issue is answerable (a hard prerequisite); ensure `ANTHROPIC_API_KEY` secret + `allow_auto_merge=true`; add `.github/dependabot.yml` (github-actions).
