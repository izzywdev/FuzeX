...
- **Cross-repo `@fuze` delegation coordination**: file/track the GitHub-issue delegations (the issue thread is the durable session), and watch for `BLOCKED:`/`DONE:` replies so the orchestrator stays unblocked.
