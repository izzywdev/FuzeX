...
5. **Secrets** — sync prints the required set. Instruct the user to set them with `gh secret set NAME` (hidden prompt) or `gh secret set NAME < gitignored-file`. **Never** accept or echo a secret value in chat. Minimum: `ANTHROPIC_API_KEY` (for `@fuze`). `FUZEINFRA_DISPATCH_TOKEN` only if the repo declares infra.
