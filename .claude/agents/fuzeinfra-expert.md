...
- `deploy-prod.yml` triggers on `helm/**` (also `argocd/**` and sealed-secrets) changes to `main`: lints the chart, runs kubeconform, registers the AppProjects/Applications, then triggers an immediate ArgoCD sync of the apps in `SYNC_APPS` and **blocks on `argocd app wait --operation --sync`** (per-app `SYNC_WAIT_TIMEOUT`, 600s) so a green run means the sync actually landed, not merely that it was requested. Waits on sync status, not health (a degraded app is a warning, not a failure). Needs the `KUBE_CONFIG` repo secret, set by `terraform apply` in `terraform/contabo/`; absent → the deploy errors out.
