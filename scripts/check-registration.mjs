...
// ── 5. the integration entry names a resolvable, same-origin location ─────────
// Convention: governance/naming-and-addressing.md
//
// FuzeMarket declared remoteEntry `https://fuzemarket.fuze.internal/dist/remoteEntry.js`.
// `.internal` is an RFC 6761 special-use name — it has no public zone and never will.
// That remote could not load for ANY user on ANY network, and nothing said so: the
// manifest is well-formed, the chart lints, the pod is healthy. The only symptom is a
// blank panel in the portal, which reads as "that product is broken".
//
// THIS CHECK DELIBERATELY OWNS NO LIST OF HOSTNAMES. Everything below is derived from
// (a) this manifest's OWN `slug` and (b) IANA/RFC rules that are true independently of
// this fleet. Adding a central table of "valid hosts per product" would create exactly
// the drift it was meant to prevent — a second place to update on every rename, silently
// wrong the moment someone forgets. A rule you can compute beats a registry you must
// maintain.
//
// ONE SEVERITY: every deviation is an error. There is no warn tier and no `|| true`.
//
// The first cut had two, on the reasoning that failing 8 of 9 products at once earns a
// gate its `|| true` within a fortnight — which is exactly what happened to gate-authz.
// That reasoning describes how a gate gets neutered, but the remedy it suggests is the
// disease: a warning tier IS a `|| true`, just spelled politely. gate-authz's trailing
// `|| true` and gate-identifier's never-set opt-in flag both produced the same outcome —
// a check that cannot fail — and this would have been the third.
//
// The owner's call, recorded here because it is the load-bearing part: fail all of them,
// fix all of them, and accept that some cannot go green until FuzeInfra lands the
// split-horizon DNS (FuzeInfra#624). A red gate on a real defect with a tracked remedy is
// a working gate. A warning nobody aggregates is not evidence for anything; it is a
// finding that has been pre-forgiven.
//
// Consequence to be honest about: this reds 8 of 9 module-federation products the moment
// it lands, and some stay red until an infra change outside their repo. That is the
// intended state, not an accident of rollout.

// RFC 6761 + RFC 2606 special-use / reserved names. Not a fleet list — these are
// non-routable by specification, anywhere, for anyone.
const UNRESOLVABLE_TLDS = ['internal', 'local', 'localhost', 'test', 'invalid', 'example']

// `manifest` is null when registration/manifest.json is absent or unparseable —
// both already reported above. Guarding rather than crashing keeps THIS check from
// masking the earlier, more useful diagnostic with a TypeError stack.
const integration = (manifest && manifest.integration) || {}
const entry = integration.remoteEntry || integration.url || ''
const slug = (manifest && manifest.slug) || ''

if (manifest && integration.type === 'module-federation' && entry && slug) {
  const hostMatch = /^[a-z][a-z0-9+.-]*:\/\/([^/:?#]+)/i.exec(entry)

  if (hostMatch) {
    const host = hostMatch[1].toLowerCase()
    const tld = host.split('.').pop()

    if (UNRESOLVABLE_TLDS.includes(tld)) {
      fail(
        `registration/manifest.json integration entry "${entry}" points at host ` +
          `"${host}", whose TLD ".${tld}" is a reserved special-use name (RFC 6761/2606). ` +
          'It has no public zone and never will, so this remote cannot load for any user ' +
          'on any network. The portal will show a blank panel and it will read as a bug ' +
          `in this product. Use the same-origin form: /apps/${slug}/remoteEntry.js`
      )
    } else {
      // Resolves, but is a second public origin: its own cert and DNS to keep alive, a
      // CORS surface, mixed content under TLS if it ever drops to http, and one more
      // thing that can be down while the portal is up.
      fail(
        `registration/manifest.json integration entry "${entry}" is cross-origin. ` +
          'A Module-Federation remote is code running inside the portal\'s page, not a ' +
          'separate site, so its remoteEntry must be mounted same-origin under the ' +
          `portal: /apps/${slug}/remoteEntry.js. A second public origin buys a CORS ` +
          'surface that need not exist, a second certificate and DNS name to keep alive, ' +
          'and mixed-content breakage the moment that host drops to http.'
      )
    }
  } else if (!entry.startsWith(`/apps/${slug}/`)) {
    // Already same-origin, but not under this product's own prefix. Derived purely from
    // this manifest's slug, so a rename re-derives and nothing central goes stale.
    fail(
      `registration/manifest.json integration entry "${entry}" is same-origin but not ` +
        `under /apps/${slug}/. The convention is /apps/<slug>/remoteEntry.js, where <slug> ` +
        'is this manifest\'s own slug — keeping every product\'s assets in one ' +
        'predictable, non-colliding prefix, and matching the build `base` the remote must ' +
        'be compiled with or every chunk 404s behind a 200 on remoteEntry.js.'
    )
  }
}
