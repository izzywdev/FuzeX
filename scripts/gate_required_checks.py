...
WHAT IT ENFORCES (each maps to an invariant in required-checks.json):
  ...
  C3 no_self_hosted        no required context runs on a self-hosted runner. A
                           `runs-on` naming a down scale set does not error, it
                           QUEUES FOREVER.
...
def audit(policy: dict, contexts: dict, repo: str) -> tuple[list, list]:
    ...
    for name in required:
        ...
        # C3 — self-hosted required contexts queue forever when the pool is down.
        runner = info["runs_on"]
        if isinstance(runner, str) and not HOSTED.match(runner) and "${{" not in runner:
            errors.append(
                f"C3 no_self_hosted_required: '{name}' is required and runs on "
                f"'{runner}'. A runs-on naming a scale set that is down does not "
                f"fail — it QUEUES FOREVER, which blocks merges with no error. "
                f"Required contexts run on GitHub-hosted runners only."
            )
    ...
    for name in dict.fromkeys(stage_promotions):
        ...
        if isinstance(runner, str) and not HOSTED.match(runner) and "${{" not in runner:
            errors.append(
                f"C6 promotion_ready: '{name}' is scheduled for promotion but runs "
                f"on '{runner}'. Required contexts run on GitHub-hosted runners "
                f"only -- a down scale set queues forever instead of failing."
            )
        hits = [m for m in VACUITY if m in info["body"]]
        if hits:
            errors.append(
